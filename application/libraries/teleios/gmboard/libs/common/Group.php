...
use teleios\gmboard\Beans\GroupNoticeBean;
...
class Group extends GmbCommon
{
    ...
    public function getGroupMessage(string $groupName, int $page = 0, int $number = 20, string $order = "DESC") : array
    {
        ...
        $libLangUtils = new LangUtils("japanese", LangUtils::MODE_REDIS);
        ...
        for ($i = 0; $i < $count; $i++) {
            switch ($messages[$i]->Idiom) {
                ...
                case 1: // join
                case 2: // withdrawal
                    $messages[$i]->Message = $libLangUtils->getMessageByCode($messages[$i]->Idiom, ["member" => $messages[$i]->Message, "group" => $groupName]);
                    break;
                case 3: // dismiss
                case 4: // joinLeader
                case 5: // joinSubLeader
                case 6: // joinMember
                case 7: // joinObserver
                case 8: // joinGuest
                    $messages[$i]->Message = $libLangUtils->getMessageByCode($messages[$i]->Idiom, ["group" => $groupName]);
                    break;
                default:
                    $messages[$i]->Message = $libLangUtils->getMessageByCode($messages[$i]->Idiom);
                    break;
            }
        }
        return $messages;
     ...
     * @return array              [description]
     */
    public function getGroupNotices(int $page = 0, int $number = 20, string $order = "DESC") : array
    {
        $offset = $page * $number;
        $daoGroupNotices = new GroupNotices();
        $notices = $daoGroupNotices->get($this->gameId, $this->groupId, $order, $number, $offset);
        return $notices;
    }
